...
demo <- "
      y ~ c*x + start(.55)*x + b1*m1 + start(.55)*m1 + b2*m2 + start(.51)*m2
      m1 ~ a1*x+start(.56)*x
      m2 ~ a2*x+start(.43)*x
      x ~~ start(1)*x
      m1 ~~ start(1)*m1
...
power.result <- power.boot(model = demo, indirect = mediation, nobs = 50, nrep = 10, nboot = 10, alpha = 0.95, parallel = "multicore") #nrep?
